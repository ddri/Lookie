...
func main() {
	...
	// Migrate companies
	if err := migrateCompanies(ctx, client); err != nil {
		log.Fatalf("Failed to migrate companies: %v", err)
	}

	// Initialize system configuration
	if err := initializeSystemConfig(ctx, client); err != nil {
...
func migrateCompanies(ctx context.Context, client *firestore.Client) error {
	...
	companiesCollection := client.Collection("companies")
	...
	for _, sqliteCompany := range sqliteCompanies {
		...
		_, err := companiesCollection.Doc(company.ID).Set(ctx, company)
		if err != nil {
...
func initializeSystemConfig(ctx context.Context, client *firestore.Client) error {
	...
	_, err := client.Collection("system").Doc(config.ID).Set(ctx, config)
	if err != nil {
